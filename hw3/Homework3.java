...
public class Homework3 {
  ...
  public static void smoosh(int[] ints) {
    ...
    int i_new = 1;//index for new array
    a[0] = ints[0];
    for (int i = 1; i<length; i++){ //detect the difference and copy to the new array
      if (!(ints[i] == ints[i-1])){
        a[i_new] = ints[i];
        i_new++;
      }
    ...
    System.arraycopy(a,0,ints,0,length); //replace the old array with the new one.
